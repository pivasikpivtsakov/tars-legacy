...
def _default_limit(size: int) -> Decimal:
    return Decimal(_DEFAULT_UNIT_PRICE_CAP * PACKAGE_UNIT_COUNT[size])
...
class PackPriceLimitRepository:
    ...
    async def get(self, *, size: int) -> Decimal:
        value = await self._redis.hget(_KEY, str(size))
        if value is None:
            return _default_limit(size)
        return Decimal(value)

    async def get_all(self) -> dict[int, Decimal]:
        raw = await self._redis.hgetall(_KEY)
        return {
            size: Decimal(raw[str(size)]) if str(size) in raw else _default_limit(size)
            for size in PACKAGE_SIZES
        }

    async def set(self, *, size: int, limit: Decimal) -> None:
